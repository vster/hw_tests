...
static size_t asize;
static u64bit src[MAXSIZE], src0[MAXSIZE], dst[MAXSIZE];
static int test_mn;
static long long d_times;
static float delay=1;
static bool verbose;
// static u64bit noresult=0;
...
void usage(void)
{
   // cout << "Processor-to-memory copy test." << endl;
   cout << "*****************************************************************************\n";
   cout << "Usage: \n";
...
void m2m_u64bit_1 (void)
{
    memcpy (dst, src, amemsize);
}

void m2m_u64bit_2 (void)
{
    memcpy (dst, src, amemsize);
}

void m2m_u64bit_3 (void)
{
    memcpy (dst, src, amemsize);
    ...
    memcpy (dst, src0, amemsize/2);
}

// Init and run one test for debug
void m2m_init()
{
    cout << "\nInit arrays for method " << test_mn << endl;
...
int main(int argc, char *argv[])
{
    ...
    cout << "Memory-to-memory copy test. 64-bit. Version 1.3\n";
    ...
    {
        cout << "\n*** Pause ***\n";
        cout << "Memory-to-memory copy test.\n";
        cout << "Press key:\n";
        cout << "   \"r\" - run test \n";
      ...
      switch (ch)
      {
        case 'r':
            ...
            while(!kbhit())
            {
                switch (test_mn)
                {
                    case 1: m2m_u64bit_1(); break;
                    case 2: m2m_u64bit_2(); break;
                    case 3: m2m_u64bit_3(); break;
                }
